...
class wumpus():
    ...
    def __init__(self,full_grid, known_grid):
        ...
        self.agent_confidence = copy.deepcopy(known_grid)
        self.agent_confidence[self.coord // self.grid_size][self.coord  % self.grid_size] -= 2
    ...
    def check_death_win(self):
        lst,lst2 = self.decode_wumpus(self.invis_grid)
    ...
    def decode_wumpus(self,decode_lst, decode_coord = -1):
        if decode_coord == -1:
        ...
        bincode = decode_lst[agent_x][agent_y]
    ...
    def shoot(self): 
        if self.arrow == True:
            ...
            if self.direction == "U":    
                ...
                if agent_y < (self.grid_size - 1):
                    newcoord += 1
            lst,lst2 = self.decode_wumpus(self.invis_grid,newcoord)
    ...
    def update_confidence(self):
        agent_confidence_new = copy.deepcopy(self.agent_confidence)

        def get_surroundings(x_imp,y_imp):
            sur = []
            for x2 in [x_imp-1, x_imp+1]:
                if x2 >= 0 and x2 < self.grid_size:
                    sur.append((x2 * self.grid_size) + y_imp)

            for y2 in [y_imp-1, y_imp+1]:
                if y2 >= 0 and y2 < self.grid_size:
                    sur.append((x_imp * self.grid_size) + y2) 
            return sur

        current_x = self.coord // self.grid_size
        current_y = self.coord % self.grid_size

        for x in range(self.grid_size):
            for y in range(self.grid_size):

                lst1,lst2 = self.decode_wumpus(self.agent_confidence,(x * self.grid_size) + y)
                if self.agent_confidence[x][y] == 1 and x == current_x and y == current_y:        # safe
                    conf_lst = get_surroundings(x,y)
                    for number in conf_lst:
                        conf_x = number // self.grid_size
                        conf_y = number % self.grid_size
                        agent_confidence_new[conf_x][conf_y] = 1

                    agent_confidence_new[current_x][current_y] = 2
                ...
                # find unsure pits
                    ...
                    for number in conf_lst:
                        conf_x = number // self.grid_size
                        conf_y = number % self.grid_size
                        agent_confidence_new[conf_x][conf_y] += 4
                ...
                # find unsure wumpus
                    ...
                    for number in conf_lst:
                        conf_x = number // self.grid_size
                        conf_y = number % self.grid_size
                        agent_confidence_new[conf_x][conf_y] += 16

                # find sure wumpus


        self.agent_confidence = agent_confidence_new
                        
    # def update_confidence_discovery(self):
    #     agent_new_x = self.coord // self.grid_size
    #     agent_new_y = self.coord % self.grid_size
    #     self.agent_confidence[agent_new_x][agent_new_y] = self.invis_grid[agent_new_x][agent_new_y] - 2

    def find_path(self,new_spot,wumpus=False,unsure=False):
        print(new_spot)
        # go to start location -> from start go to other location - > remove overlap????
        # expand current position with visited -> find route to other location

    def play_agent(self):
        print(self.agent_confidence)
        print(self.known_grid)
        print(self.invis_grid)
        self.update_confidence()
        
        # explore safe 
        move = True
        flat_list = [item for sublist in self.agent_confidence for item in sublist]

        if 1 in flat_list:      # safe spot
            self.find_path(flat_list.index(1)) # go to each safe spot
            self.forward()
                    
        elif 32 in flat_list:  # kill wumpus
            self.find_path(flat_list.index(32),wumpus=True)
            
        # find unsure spot [4 or 16]
        elif 4 in flat_list or 16 in flat_list: # do a random "safe" move [ future to be logic]
            number1 = flat_list.index(4) if 4 in flat_list else 99
            number2 = flat_list.index(16) if 16 in flat_list else 99
            
            if number1 < number2:
                self.find_path(number1,unsure=True)
            else:   
                self.find_path(number2,unsure=True)

        else:  
            print("no other options")
            self.find_path((self.grid_size ** 2) - self.grid_size)
            self.leave()
